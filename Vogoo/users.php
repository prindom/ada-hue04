...
EOF;
			...
			foreach($result as $row) //while ($i < $k && $row = $vogoo->db->sql_fetchrow($result))
			{
				if($i>=$k) break;
...
EOF;
	...
	function get_product_info($product_id){
		...
		$sql = <<<EOF
SELECT * FROM vogoo_products where product_id={$product_id}
EOF;
		if ( !($result = $vogoo->db->sql_query2($sql)) )
		{
			return false;
		...
		//$product_genres = $row['product_genres'];
		//return $row;
		return $result;
	}
}
